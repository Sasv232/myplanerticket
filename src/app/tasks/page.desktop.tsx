...
export function TasksPageDesktop() {
  ...
  if (loading) {
    return <div className="flex h-64 items-center justify-center"><div className="text-[var(--secondary)]">╨Ч╨░╨│╤А╤Г╨╖╨║╨░...</div></div>;
  }
  ...
  return (
    <div>
      <Header
        title="╨Ч╨░╨┤╨░╤З╨╕"
        description={`╨Т╤Б╨╡╨│╨╛: ${stats.total} ┬╖ ╨Р╨║╤В╨╕╨▓╨╜╤Л╤Е: ${stats.todo + stats.inProgress}`}
        actions={
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => setShowTemplates(!showTemplates)}>
              <BookTemplate className="h-4 w-4" /> ╨и╨░╨▒╨╗╨╛╨╜╤Л
            </Button>
            <Button onClick={() => { setEditingTask(undefined); setFormOpen(true); }}>
              <Plus className="h-4 w-4" /> ╨Э╨╛╨▓╨░╤П ╨╖╨░╨┤╨░╤З╨░
            </Button>
          </div>
        }
      />
          ...
          <CardContent className="p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" onClick={saveAsTemplate} disabled={selectedIds.size === 0 || !templateName.trim()}>
                ╨б╨╛╤Е╤А╨░╨╜╨╕╤В╤М ╨▓╤Л╨▒╤А╨░╨╜╨╜╤Л╨╡ ({selectedIds.size})
              </Button>
            </div>
            {templates.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {templates.map((t) => (
                  <div key={t.id} className="flex items-center gap-1 rounded-lg border border-[var(--border)] px-3 py-1.5 text-sm">
                    ...
                    <button onClick={async () => { await fetch(`/api/templates?id=${t.id}`, { method: "DELETE" }); fetchTemplates(); }} className="ml-1 text-[var(--error)] text-xs">тЬХ</button>
                  </div>
                ))}
              </div>
            )}
      ...
      {showBulkBar && (
        <Card className="mb-4 border-[var(--accent)]/30">
          <CardContent className="flex items-center gap-3 p-3">
            <span className="text-sm text-[var(--secondary)]">╨Т╤Л╨▒╤А╨░╨╜╨╛: {selectedIds.size}</span>
            <Button size="sm" variant="outline" onClick={selectAll}>
              {selectedIds.size === filteredTasks.length ? "╨б╨╜╤П╤В╤М ╨▓╤Л╨┤╨╡╨╗╨╡╨╜╨╕╨╡" : "╨Т╤Л╨▒╤А╨░╤В╤М ╨▓╤Б╨╡"}
            </Button>
            <select onChange={(e) => { if (e.target.value) bulkAction("status", e.target.value); e.target.value = ""; }} className="h-8 rounded border border-[var(--border)] bg-[var(--surface)] px-2 text-xs">
              <option value="">╨б╤В╨░╤В╤Г╤Б...</option>
              <option value="todo">╨Ъ ╨▓╤Л╨┐╨╛╨╗╨╜╨╡╨╜╨╕╤О</option>
              <option value="in_progress">╨Т ╤А╨░╨▒╨╛╤В╨╡</option>
              <option value="done">╨Т╤Л╨┐╨╛╨╗╨╜╨╡╨╜╨╛</option>
            </select>
            <select onChange={(e) => { if (e.target.value) bulkAction("priority", e.target.value); e.target.value = ""; }} className="h-8 rounded border border-[var(--border)] bg-[var(--surface)] px-2 text-xs">
              <option value="">╨Я╤А╨╕╨╛╤А╨╕╤В╨╡╤В...</option>
              <option value="low">╨Э╨╕╨╖╨║╨╕╨╣</option>
              <option value="medium">╨б╤А╨╡╨┤╨╜╨╕╨╣</option>
              <option value="high">╨Т╤Л╤Б╨╛╨║╨╕╨╣</option>
              <option value="urgent">╨б╤А╨╛╤З╨╜╤Л╨╣</option>
            </select>
            <Button size="sm" variant="destructive" onClick={() => bulkAction("delete")}>
              <Trash2 className="h-3.5 w-3.5" /> ╨г╨┤╨░╨╗╨╕╤В╤М
            </Button>
            <Button size="sm" variant="ghost" onClick={() => { setSelectedIds(new Set()); setShowBulkBar(false); }}>╨Ю╤В╨╝╨╡╨╜╨░</Button>
          </CardContent>
        </Card>
      )}

      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Card><CardContent className="flex items-center gap-3 p-4"><ListTodo className="h-5 w-5 text-[var(--accent)]" /><div><p className="text-2xl font-bold">{stats.todo}</p><p className="text-xs text-[var(--secondary)]">╨Ъ ╨▓╤Л╨┐╨╛╨╗╨╜╨╡╨╜╨╕╤О</p></div></CardContent></Card>
        <Card><CardContent className="flex items-center gap-3 p-4"><Clock className="h-5 w-5 text-[var(--warning)]" /><div><p className="text-2xl font-bold">{stats.inProgress}</p><p className="text-xs text-[var(--secondary)]">╨Т ╤А╨░╨▒╨╛╤В╨╡</p></div></CardContent></Card>
        <Card><CardContent className="flex items-center gap-3 p-4"><CheckCircle className="h-5 w-5 text-[var(--success)]" /><div><p className="text-2xl font-bold">{stats.done}</p><p className="text-xs text-[var(--secondary)]">╨Т╤Л╨┐╨╛╨╗╨╜╨╡╨╜╨╛</p></div></CardContent></Card>
        <Card><CardContent className="flex items-center gap-3 p-4"><AlertTriangle className="h-5 w-5 text-[var(--error)]" /><div><p className="text-2xl font-bold">{stats.urgent}</p><p className="text-xs text-[var(--secondary)]">╨б╤А╨╛╤З╨╜╤Л╤Е</p></div></CardContent></Card>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-1 ml-auto">
          ...
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortBy)} className="h-8 rounded border border-[var(--border)] bg-[var(--surface)] px-2 text-xs">
            <option value="created">╨Я╨╛ ╨┤╨░╤В╨╡ ╤Б╨╛╨╖╨┤╨░╨╜╨╕╤П</option>
            <option value="date">╨Я╨╛ ╨┤╨╡╨┤╨╗╨░╨╣╨╜╤Г</option>
            <option value="priority">╨Я╨╛ ╨┐╤А╨╕╨╛╤А╨╕╤В╨╡╤В╤Г</option>
            <option value="name">╨Я╨╛ ╨╜╨░╨╖╨▓╨░╨╜╨╕╤О</option>
          </select>
          {!showBulkBar && (
        ...
        {filteredTasks.length === 0 ? (
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12 text-[var(--secondary)]">
              ...
              <p>╨Ч╨░╨┤╨░╤З ╨┐╨╛╨║╨░ ╨╜╨╡╤В</p>
              <Button variant="ghost" className="mt-2" onClick={() => { setEditingTask(undefined); setFormOpen(true); }}>╨б╨╛╨╖╨┤╨░╤В╤М ╨┐╨╡╤А╨▓╤Г╤О ╨╖╨░╨┤╨░╤З╤Г</Button>
            </CardContent>
          </Card>
        ) : (
