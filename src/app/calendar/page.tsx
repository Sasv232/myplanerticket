...
export default function CalendarPage() {
  ...
  const calendarGrid = (cellClass: string, dayHeaderClass: string) => (
    <div className="grid grid-cols-7 gap-px rounded-xl border border-[var(--border)] bg-[var(--border)] overflow-hidden">
      {["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"].map((day) => (
        <div key={day} className={`${dayHeaderClass} p-2 text-center text-xs font-medium text-[var(--secondary)]`}>
          {day}
        </div>
      ))}
      {days.map((day) => {
        ...
        return (
          <div
            key={day.toISOString()}
            className={`${cellClass} bg-[var(--card)] p-2 ${today ? "ring-2 ring-[var(--accent)] ring-inset" : ""}`}
          >
    ...
    <>
      ...
      <div className="md:hidden space-y-4">
        ...
        <div className="mobile-widget-card p-4">
          ...
          {calendarGrid("min-h-[60px] rounded-xl", "")}
        </div>
      </div>
    </>
  );
