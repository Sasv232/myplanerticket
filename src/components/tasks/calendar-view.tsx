    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => nav("left")} className="flex h-8 w-8 items-center justify-center rounded-lg hover:bg-[var(--surface)] transition-colors">
            <ChevronLeft className="h-4 w-4" />
          </button>
          <span className="min-w-[140px] text-center text-sm font-medium">{viewLabel}</span>
          <button onClick={() => nav("right")} className="flex h-8 w-8 items-center justify-center rounded-lg hover:bg-[var(--surface)] transition-colors">
      ...
      {viewMode === "month" && (
        <div className="grid grid-cols-7 gap-px rounded-xl border border-[var(--border)] bg-[var(--border)] overflow-hidden">
          {["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"].map((day) => (
            <div key={day} className="min-h-[80px] p-2 text-center text-xs font-medium text-[var(--secondary)] bg-[var(--background)]">
              {day}
            </div>
          ))}
          {days.map((day) => {
            ...
            return (
              <div
                key={day.toISOString()}
                className={`min-h-[80px] bg-[var(--card)] p-2 ${today ? "ring-2 ring-[var(--accent)] ring-inset" : ""}`}
              >
                <p className={`mb-1 text-xs font-medium ${today ? "text-[var(--accent)]" : ""}`}>
                  {format(day, "d")}
                </p>
                <div className="space-y-0.5">
                  {dayTasks.slice(0, compact ? 2 : 3).map((task) => (
                    <div
                      key={task.id}
                  ...
                  {dayTasks.length > (compact ? 2 : 3) && (
                    <p className="text-[10px] text-[var(--secondary)]">+{dayTasks.length - (compact ? 2 : 3)}</p>
                  )}
                </div>
              </div>
            );
      ...
      {viewMode === "week" && (
        <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] overflow-hidden">
          <div className="grid grid-cols-8 border-b border-[var(--border)]">
            ...
            {weekDays.map((d) => (
              <div key={d.toISOString()} className={`flex flex-col items-center min-h-[40px] border-b border-[var(--border)] p-2 ${isToday(d) ? "bg-[var(--accent)]/5" : ""}`}>
                <span className="text-[10px] text-[var(--secondary)]">{format(d, "EEE", { locale: ru })}</span>
                <span className={`text-sm font-semibold ${isToday(d) ? "text-[var(--accent)]" : ""}`}>{format(d, "d")}</span>
              </div>
            ))}
          </div>
            ...
            {TIME_SLOTS.map((hour) => (
              <div key={hour} className="grid grid-cols-8 border-b border-[var(--border)] last:border-b-0">
                ...
                {weekDays.map((day) => {
                  ...
                  const today = isToday(day);
                  return (
                    <div key={day.toISOString()} className={`min-h-[36px] p-0.5 ${today ? "bg-[var(--accent)]/5" : ""}`}>
                      {slotTasks.map((task) => (
                        <div key={task.id} className="rounded bg-[var(--accent)]/10 px-1 py-0.5 text-[9px] mb-0.5 truncate" title={task.title}>
                          {task.title}
                        </div>
                      ))}
                    </div>
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] overflow-hidden">
          <div className={`border-b border-[var(--border)] p-3 ${isToday(currentDate) ? "bg-[var(--accent)]/5" : ""}`}>
            <div className="text-center">
              <span className="text-xs text-[var(--secondary)]">{format(currentDate, "EEEE", { locale: ru })}</span>
          ...
          <div className="max-h-[400px] overflow-y-auto">
            {TIME_SLOTS.map((hour) => {
              ...
              return (
                <div key={hour} className="grid grid-cols-[50px_1fr] border-b border-[var(--border)] last:border-b-0 min-h-[44px]">
                  <div className="border-r border-[var(--border)] p-1 text-[10px] text-[var(--secondary)] text-right pt-2">
                    {String(hour).padStart(2, "0")}:00
                  </div>
                  <div className="p-1 space-y-0.5">
                    {hourTasks.map((task) => (
                      <div key={task.id} className="rounded bg-[var(--accent)]/10 px-2 py-1 text-xs">
